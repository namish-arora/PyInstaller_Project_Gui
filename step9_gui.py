...
output_text = tk.Text(output_frame, height=3, width=50, wrap="word", bd=2, font=("Aerial", 10), fg="blue")
...
#     output_path = output_text.get("1.0", tk.END).strip()
#     if os.path.isdir(output_path):
#         os.startfile(output_path)
#     else:
#         messagebox.showerror("Error", "Invalid output path. Please run PyInstaller first.")

# open_folder_button = tk.Button(output_frame, text="Open Output Folder", command=open_output_folder, fg="black", bg="aqua", width=20, font=("Arial", 10))
...
# Wrapper function to run PyInstaller and add text area
def wrapper_function():
    
    output_frame.pack(before = package_button,anchor="w", padx=20, pady=(0, 5))  # Show output frame
    Label(output_frame,text="Output path:",font=("Aerial",12)).pack(side=tk.LEFT, padx=(0, 10))
    folder_image = Image.open("folder_icon.png").resize((20, 20), Image.Resampling.LANCZOS)
    folder_image = ImageTk.PhotoImage(folder_image)
    folder_icon_label = tk.Label(output_frame, image=folder_image)
    folder_icon_label.image = folder_image  # Keep a reference to avoid garbage collection
    folder_icon_label.pack(side=tk.LEFT, padx=(0, 10))  # Add some space between icon and text
